...
둘째, 스테이트리스 커뮤니케이션 방식으로 요청간 분리되어있습니다. <br/>\
...
'의료 프로젝트와 일반 프로젝트의 PM의 역할은 대동소이 하다고 생각합니다. <br/>\
프로젝트 관리자로써 통합, 범위, 일정 관리등 프로젝트의 성공을 위해 전반적인 관리를 수행해야 하고, <br/>\
의료 프로젝트에서 PM은 자신이 수행하는 프로젝트가 국민의 생명과 안전을 지키는 프로젝트임을 기억하며 사명감을 더해 마음 가짐을 가져야 한다고 생각합니다.\
',
...
'의료 업계는 국민의 생명과 안전을 담보로 하기 때문에 어쩔 수 없이 기술 수용에 소극적일 수 밖에 없다고 생각합니다. <br/>\
이를 극복하기 위해선 정책 측면과 산업 측면으로 설명 드릴 수 있습니다. <br/>\
먼저 정책 측면으론, 의료 IT 기술 적용에 따른 수가 인센티브제를 적용 할 수 있습니다. <br/>\
다음으로 산업 측면으론, 기술에 대한 인증 제도를 확대 함으로써 의료 IT 기술 융합 촉진이 가능할 것이라 생각듭니다.\
',
...
2010년대 초반에는 병원내부 전사적 전산화를 위해 HIS가 도입 되었고, <br/>\
2010년대 후반에는 HIS의 OS 종속성 탈피를 기반으로 발전하였습니다. <br/>\
...
'G-Cloud 채택 조건으론 클라우드 보안 인증제와 클라우드 서비스 인증제를 받은 제품이 그 조건입니다.\
',

'첫째, 개념 차이점입니다. EAI 는 표준화 되지 않은 이기종 시스템간의 연동 솔루션이고, ESB는 EAI 복잡성 해결 위해 등장한 SOA 구조의 연동 솔루션, API GW는 내/외부 시스템 연동 솔루션입니다. <br/>\
둘째, 기술 차이점입니다. EAI 는 Adapter를 중심으로 동작하고, ESB는 SOAP 기반의 Protocol을 중심으로 동작하고, API GW 는 REST 기술을 기반으로 동작합니다.\
',

'첫째, 정보 유형 관점입니다. 의료 마이데이터는 정형 데이터와 비정형 데이터를 모두 포함한 데이터를 비식별 처리 하여야 하고, 금융 데이터는 주로 정형 데이터만 비식별 처리와 같은 취급 데이터 유형의 차이점이 존재합니다. <br/>\
둘째, 비식별 수준 관점입니다. 의료 마이데이터는 가명 처리와 더불어 익명처리 까지 고려해야할 대상이 존재하는 반면, 금융 마이데이터는 대부분 가명처리 수준으로 처리합니다. \
',

'먼저 G-Cloud 구성시 고려사항은 정책적, 기술적 고려사항으로 말씀 드릴수 있습니다. <br/>\
먼저 정책적 고려사항은 G-Cloud는 인가된 정부 및 공공기관만 사용할 수 있기 때문에, 이에 해당되는지 확인, 승인 절차가 필요합니다. <br/>\
다음으로 기술적 고려사항은 해당 G-Cloud의 클라우드 보안 인증 수준과 클라우드 서비스 인증 수준을 확인하여 사업 계획과 비교 대조 해보아야 합니다. <br/><br/>\
이로 미루어 볼때, 일반 클라우드와 구성시 차이점은 첫째, 참여 자격의 제한이 있다는 점과 둘째, G-Cloud 에서 사용가능한 상용 제품의 제한이 존재하는 점을 말씀드릴 수 있습니다. 이에 대해 추가 설명 해도 되겠습니까? <br/><br/>\
실제로 G-Cloud는 SW 국산화의 취지 또한 내포하기 때문에 RDBMS 경우 주로 사용되는 Oracle 은 직접 지원이 불가능 하고, 국산 RDBMS 인 Tibero 사용 유도를 하는 현황 또한 존재합니다.\
',

'의무기록 표준화시 고려사항을 기술적 관점과 비즈니스적 관점으로 나누어 설명드리겠습니다. <br/>\
먼저 기술적 고려사항입니다. IoT, AI 등 의료 IT 기술 영역 확장으로 인한 의료 기술 표준의 확장성을 고려한 표준화 설계 반영을 고려해야합니다. <br/>\
다음으로 비즈니스적 고려사항입니다. 의료 데이터 Silo 벽을 허물기 위해 수가 정책 유도, EMR 인증제도 등 정책,제도 차원에서의 접근을 고려해야합니다.\
',

'Optimizer 는 RBO, CBO 두 유형이 있지만, 제가 실무에 사용한 CBO Optmizer의 절차와 적용사례통한 정량적지표를 말씀 드리겠습니다. <br/>\
먼저 절차입니다. 절차는 후보군이 될만한 실행계획 탐색, 딕셔너리 정보를 활용한 비용 산정, 실행계획중 최적의 비용을 최종 선택하는 절차를 따르게 됩니다. <br/>\
실무에선 Optimizer 의 경로를 따르지 않고 Hint 키워드를 통한 강제 실행계획 SQL Tunning을 적용시켜 5개의 Table Join, 1만 건 Output Data Running Time 7분 가량 걸리던 Query를 30초 미만으로 단축시키는 성과를 경험한 적이 있습니다.\
',

'우선 AI 구급차는 응급환자의 Golden Time 을 지키기 위해 구급차의 최적경로를 AI 를 통해 추천해주는 기술을 말합니다. 이에 대한 구현시 주요 항목을 관리적, 기술적 측면으로 설명드리겠습니다. <br/>\
우선 관리적 측면 주요항목입니다. 의료기관 정보항목과 도로정보 항목으로 구성되며, 이를 통해 State 값을 관리합니다. <br/>\
다음으로 기술적 측면 주요항목입니다. 관리적 측면 주요항목은 구동 알고리즘 선정이며, 선정 대상엔 순차적 경로생성에 주로 사용되는 강화학습중 실시간 Q-Learning 알고리즘과 DQN 알고리즘이 후보대상이었으며 Q-Learning 알고리즘으로 채택한 것으로 알고있습니다.\
',

'빅데이터 생명주기는 생성, 저장, 활용입니다. 추가로 제 경험을 말씀드려도 되겠습니까? <br/><br/>\
제가 참여한 암 빅데이터 플랫폼 구축당시 진료 데이터를 실시간 수집하여 이를 정제하는 생성 단계를 거치고, <br/>\
국립 암센터의 G-Cloud에 규격에 맞춰 저장한 후, 국립 암 센터에서 제공하는 API를 통해 예측 기반의 진료를 위해 데이터를 활용하였습니다.\
',

'위드 코로나와 원격진료에 대한 제 생각은 원격 진료를 유지해야 한다 입니다. <br/>\
코로나 19로 인해 그간 부진했던 원격 진료 사업이 폭발적으로 발전했고, 또한 연간 사용건이 270만건을 넘어섰습니다. 이는 의료 품질을 높일 수 있는 차원이라 생각 되기 때문에, 법,제도 개정을 통해 산업 생태계 존속이 필요하다고 생각합니다.\
',

'진료정보는 생성 > 저장 > 활용 3단계로 생성 및 공유 됩니다. <br/>\
첫째, 생성 단계 입니다. 지역 의료기관에서 진료한 내용의 정보가 HL7-CDA 표준을 준수하여 생성됩니다. <br/>\
둘째, 저장 단계 입니다. 표준을 준수한 데이터를 진료정보 공유시스템에 저장하게 됩니다. <br/>\
셋째, 활용 단계 입니다. 진료정보 공유시스템에 저장된 문서를 기관간 공유, 활용하여 기관간 정보공유를 완료합니다. <br/><br/>\
안전한 정보 전송을 위한 개인정보 비식별 처리 기법은 가명, 총계, 삭제, 범위, 마스킹 등을 통해 수행하였습니다.\
',

'감리의 종류는 정기 감리, 상주감리, 추가감리가 있고, 감리 절차는 예비조사, 현장 감리, 시정조치 확인 절차로 구성됩니다. 진료정보 공유시스템에서 감리 대응한 내용을 추가 설명 드려도 되겠습니까? <br/><br/>\
진료정보 공유시스템은 30억 예산 규모로 3단계 감리 대상이었고, 요구조사 이후, 분석,설계 이후, 개발 이후 종 3번에 거쳐 감리를 받았습니다.\
',

'마이 헬스웨이 플랫폼은 의료 마이데이터 전용 플랫폼입니다. <br/>\
국민의 건강증진을 비전으로 두고, 마이데이터 생태계 조성을 목표로 하여, 3대 추진전략과 12대 추진과제로 구성되어 있습니다. 이에 추가 설명 드려도 되겠습니까? <br/><br/>\
점진적 확대, 안전한 플랫폼, 민관협력 기반으로 추진 전략을 세우고 이를 통한 의료분야 마이데이터 도입기반을 마련하는 플랫폼입니다. 제가 실무적으로 참여했던 진료정보공유시스템, 암빅데이터 플랫폼, 응급의료 시스템 구축 프로젝트가 이에 속하는 프로젝트입니다.\
',

'공공건강데이터, 병원의료데이터, 개인겅강데이터, 유전체 정보 등이 있고, 요소 기술로는 EHR 표준화, 데이터전송, 개인인증, 결제, 보안, AI, 클라우드, 5G 등이 있습니다.\
',

'PM의 가장 중요한 역량에 대해 내부적 측면과 외부적 측면으로 나누어 설명드리겠습니다. <br/>\
먼저 프로젝트 팀 내부적 측면입니다. 성공적인 프로젝트를 위해선 PM의 솔선수범하는 자세와 프로젝트 팀원에게 적절한 보상체계를 갖추어 프로젝트 팀에 활기를 불어 넣는 것이 중요하다고 생각합니다. <br/>\
다음으로 외부적 측면입니다. PM은 프로젝트의 선장과도 같으므로 다양한 이해관계자와의 의사소통 능력, 조정 능력 등을 위해 기본적으로 필요한 커뮤니케이션 능력이 중요하다고 생각합니다.\
',

'해당 사업은 국립 암 센터를 중심으로 10개의 암 빅데이터 플랫폼 중 한 지역의 구축 사업에 PL로 참여하였습니다. <br/>\
제가 직면했던 데이터 속도 문제중 하나는 상병명이 복합적으로 구성되어 있는 복합 상병 문제로 인해 속도 문제가 있었습니다. <br/>\
이를 해결하기 위한 방법으로 기술적 방법으로 복합 상병 분리 모듈 개발과 관리적 방법으로 지역 의료기관의 전송 스키마 제약이 있었으며, 의료기관 특성상 복합 상병 허용이 불가피 했기 때문에, 이를 위한 데이터 분석 모듈을 자체 개발하여 데이터 성능 이슈를 해결하였습니다.\
',

'DQM 표준에 의거하여 품질 기준을 말씀드리겠습니다. <br/>\
정확성, 일관성, 유용성, 접근성, 적시성, 보안성이 존재합니다. 이를 위한 실무 적용에 대한 추가 설명 드려도 되겠습니까? <br/><br/>\
이를 유지하기 위해 데이터 값, 구조, 관리 프로세스로 나누고 이해관계자 즉, CIO, DA, Modeler, DBA, 사용자 별 관리 위한 역할 분담표를 기반으로 수행하였습니다.\
',

'먼저 개념 차이점 입니다. ESB 는 SOA 기반의 Bus 토폴로지 서비스 연계 방식이고, EAI 는 이기종 애플리케이션 간 연계 방식입니다. <br/>\
다음으로 기술 차이점입니다. ESB 는 SOAP 기반의 통신으로 연계하고, EAI 는 Adapter 방식으로 연계합니다.\
',

'첫째, 의료영상 분석입니다. 다른 말로 TeleRadiology 라고도 합니다. CT, MRI, X-Ray 등의 영상을 CNN, RNN등을 활용하여 분석하는 기술입니다. <br/>\
둘째, 전자기록 입니다. 의료현장에서 벌어지는 모든 음성정보를 문자로 전사하여 인공지능이 기록하는 것을 의미합니다. <br/>\
셋째, 예측기반 진료입니다. 병원 데이터를 기반으로 질병 예측, 예방, 치료등 접목이 가능합니다. 이를 통한 제2의 코로나19사태를 막을 수 있으리라 생각됩니다.\
',

'EHR은 Electronic Health Record로, EMR 정보에서 담지 않았던 개인 건강정보를 포함한 기록체계이고, <br/>\
PHR은 Personal Health Record로, 기존 의료기관 중심 건강관리에서 각 개인이 자신의 건강관리 주체가 되는 패러다임을 지원하는 시스템을 의미합니다.\
',

'EMR 고도화 프로젝트에서 수행했던 DB 암호화는 Linux OS 환경 내에서의 DB 암호화였고, 암호화를 위한 키는 Root 계정에 보관하여 관리하였습니다.\
',

'첫째, 개념 차이점입니다. RDBMS 는 관계 기반의 데이터베이스 관리 시스템이고, NoSQL 은 RDBMS 의 한계 극복을 위한 유연성 기반의 분산 데이터 관리 시스템입니다. <br/>\
둘째, 취급 데이터 차이점입니다. RDBMS 는 정형데이터를 취급하고, NoSQL은 비정형 데이터까지 취급합니다. <br/>\
셋째, 이론 차이점입니다. RDBMS 는 ACID 이론 기반으로 설계되고, NoSQL은 CAP, PACELC 이론을 기반으로 설계됩니다. <br/>\
마지막으로 사례입니다. RDBMS 는 Oracle, MariaDB 등이 존재하고, NoSQL 은 HBase, Hadoop, MongoDB등이 존재합니다.\
',

'DB 튜닝은 SW 튜닝과 HW 튜닝으로 나뉩니다. <br/>\
SW 튜닝은 Index 설계, Schema 설계, Table 설계 등이 있고, <br/>\
HW 튜닝은 Buffer Size, Memory Size, Disk SIze 변경 등이 있습니다. 이에 관련된 실무 경험을 말씀 드려도 되겠습니까? <br/><br/>\
우선 DB 튜닝을 위해선 담당자 인터뷰를 선 수행하고, 이를 기반으로 문제점을 예측합니다. S/W 튜닝을 하고 H/W 튜닝을 하는 것이 순서입니다.\
',
];