...
# 증거유형 <br/>\
1. 자동생성 여부 <br/>\
- 자동생성 증거 : 인터넷 기록, 방홥겨 로그, OS 로그 <br/>\
...
# 수집 기술 <br/>\
1. 저장 매체, 메모리 <br/>\
- 데이터 이미징, 파일 시스템 로그 분석, 메모리 덤프, 레지스트리 하이브 <br/><br/>\
2. 시스템, OS 데이터 <br/>\
- MBR분석, ThumbCache, 휴지통 분석 <br/><br/>\
3. 디지털 데이터 <br/>\
- 시스템 로그, 백업데이터 분석, 윈도우 레지스트리, 프리패치 분석 <br/><br/>\
4. 네트워크, 통신 Data <br/>\
- 네트워크 역추적, 허니넷, 웹 캐시 확보 위한 프록시, 트래픽 서버 수취 <br/><br/>\
5. 범죄 프로파일링 기반 <br/>\
- 타임라인 분석, 프로파일링, SW 역공학, 회계부정탐지기술 <br/><br/>\
# 분석 기술 <br/>\
- 시계열 분석 : 파일 변경/생성/사용/삭제된 시간별로 분류 / EnCase 도구 <br/>\
- 폴더브라우징 : 파일 및 디렉토리의 액세스 시간 열람 <br/>\
- 고급 검색 : 파일 슬랙영역, 삭제된 파일 또는 압축파일 검색 <br/>\
- 로그 분석 : 웹브라우저, 메신저, FTP등 다양한 응용 프로그램 사용 흔적 조사 <br/><br/>\
* 라이지움 88회 응용 3교시 5번\
',
...
# ISO/IEC 20889 기준, 가명처리 기법 (해재암통일가무)<br/>\
<img src = "./img/ISOIEC20889_Tech.png" style = "max-width:100%; height:auto;"><br/><br/>\
# 시계열 데이터 결합 절차 <br/>\
...
// OECD Privacy 8원칙
'# 정의 : OECD 프라이버시 가이드라인에서 제시하는 8가지 원칙 (2013.7.11. 최종 채택) <br/>\
- 변화하는 기술, 시장과 이용자 형태, 증가하는 사이버 정체성의 중요성에 따라 개정 <br/><br/>\
# 내용 <br/>\
...
- 외부로부터 입력된 값을 검증하지 않고 입출력 합수의 포맷 스트링을 그대로 사용하여 메모리 내용을 읽거나 악성 쉘을 실행시키는 공격 <br/><br/>\
# 공격 절차 <br/>\
...
// Zero Trust
'# 정의 : 모든 자산 위험 요소, 무신뢰 기반 과립형 경계 보안 모델 <br/>\
- 모든 네트워크 자산을 위험 요소로 전제, 모든 데이터 모니터링, 접근 경로 통제, 액세스 권한 제어 통한 무신뢰 기반 과립형 경계 보안 모델 <br/><br/>\
# 부각배경 : 초연결 환경, 내부 사용자 의한 보안 사고 증가 <br/><br/>\
# 제로 트러스트 모델 적용 방안 <br/>\
1. 중요 데이터 식별 : 자산 중요도, 데이터 민감도 <br/>\
2. 민감 데이터 흐름 설계 : 데이터 Flow 최적화 <br/>\
3. Micro Perimeters 설계 : 보안 경계 설정, 정책/Tool 구성 <br/>\
4. 보안분석, 모니터링 : 지속 모니터링 체계 구축 <br/>\
5. 보안자동화, Orchestration : 자동화 기반 운영 효율화 <br/><br/>\
# 기술요소 <br/>\
1. 보안 강도 강화 측면 <br/>\
- 지능형 WAF : 의미 분석, 구문 분석 수준의 데이터 분석 수행 <br/>\
- DevSecOps, SIEM 통합 : IaaS/PaaS/SaaS 환경 배포, 서비스 보안강화 <br/><br/>\
2. 사용성, 운용성 제고 측면 <br/>\
- Secure SSO : 데이터 접근 제한 및 사용자 편의성 동시 고려 <br/>\
- Column 단위 암호화 : 컬럼 단위 제한, 최소 범위, 최소 인원 대상, 최단 시간 노출 <br/><br/>\
