...
// Race Condition
'# 정의 : 여러 프로세스 / 공유 자원 / 동시 접근 / 비정상적 상태 <br/>\
- 여러 프로세스들이 공유된 자원에 동시 접근을 시도할 때 접근의 타이밍이나 순서 등이 결과 값에 영향을 줄 수 있는 비정상적 상태 <br/><br/>\
# 발생 조건 <br/>\
- 자원할당 측면 : 프로세스 동작이 단일 자원 강제 할당 <br/>\
- 자원사용 측면 : 복수 프로세스 동시 자원 선점 <br/><br/>\
# 발생 사례 <br/>\
- Kernel 수행 : Running 중 Interrupt 발생 <br/>\
- 문맥 교환 : Process 커널모드 수행 중 문맥 교환 발생 / 커널 모드일 때 CPU 선점 x 처리 <br/>\
- 공유 메모리 : 멀티 프로세서 공유 메모리 저장 / 한 번에 하나의 CPU 만 커널 진입 <br/><br/>\
# 해결 방법 충족 조건 <br/>\
- 상호 배제 : 임계 영역 동시 접근 불가 <br/>\
- 진행 : Empty 임계 영역 사용 가능 <br/>\
- 유한 대기 : 프로세스 임계 영역 사용 횟수 한계 제한 <br/><br/>\
# 해결 방법 <br/>\
- 상호배제 : 피터슨 알고리즘, 하드웨어 명령어 상태점검 <br/>\
- 동기화 : 세마포어, 모니터, Spin Lock <br/><br/>\
* ITPE 9회 응용 2교시 6번\
',
