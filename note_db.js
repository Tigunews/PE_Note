...
'[데이터 품질]- DQM3',
'[데이터 품질]- ISO 11179',
'[데이터 품질]- TTAK.KO-10.0555',
'[데이터 품질]- ISO/IEC 20943-5',
'[데이터 품질]- DQC',
'인덱스',
'다차원 색인구조',
'[다차원 색인]- PAM',
...
// 데이터 품질
'# 데이터 품질 인증 <br/>\
- DQC-V : 심사 기준(도메인, 업무규칙), 인증 레벨(플골실) <br/>\
- DQC-M : 심사 기준(정확, 일관, 유용, 접근, 적시, 보안), 인증 레벨(도정통정최 1~5) <br/>\
- DQC-S : 심사 기준(접암작취), 인증 레벨(접암작취) <br/><br/>\
# 품질 진단 프로세스 <br/>\
- 계획 수립 : 프로젝트 정의서, WBS <br/>\
- 대상 정의 : 데이터 모델, 표준, 메타데이터, 누락 값, 허용 범위, 패턴 분석 <br/>\
- 품질 측정 : 업무규칙 단위 오류율, 테이블, 컬럼 중요도 <br/>\
- 결과 분석 : 데이터 입력, 흐름 통제, 품질 지수, 점수 산출 <br/>\
- 품질 개선 : 시급성, 우선순위, 품질관리 피드백 <br/><br/>\
# 데이터 표준화 프로세스 <br/>\
- 요소 : 명칭, 정의, 형식 규칙 <br/>\
- 항목 : 용단도코 <br/>\
- 절차 : 요구 수집 > 원칙 성립 > 표준정의 (단어, 도메인, 코드, 용어, 요소) > 검토 > 공표 <br/><br/>\
# 데이터 품질관리 프레임워크 <br/>\
- 대상 : 값, 구조, 프로세스 <br/>\
- 조직 : CIO, DA, Modeler, DA, DBA, User <br/><br/>\
* ITPE 9회 관리 3교시 6번\
',
...
// 나이브 베이즈 분류
'# 정의 : 베이즈 정리 적용 / 확률 분류기 <br/>\
- 베이즈 정리에 기반한 통계 분류기법 <br/><br/>\
# 구성요소 <br/>\
- 나이브(Naive) : 예측한 특징이 상호 독립적 가정하에 확률 계산 단순화 <br/>\
- 베이즈(Bayes) : 입력 특징이 전체 확률 분포 대비 특정 클래스 속할 확률 계산 <br/><br/>\
# 장단점 <br/>\
1. 장점 <br/>\
- 간단, 빠름, 효율적 <br/>\
- 잡음, 누락 데이터 처리 <br/>\
- 훈련 데이터 크기 상관 없이 동작 <br/>\
- 예측을 위한 추정 확률 쉽게 계산 <br/><br/>\
2. 단점 <br/>\
- 모든 특징 동등, 독립이라는 가정 잘못 된경우 발생 <br/>\
- 수치 특정이 많은 데이터셋에는 이상적이지 않음 <br/>\
- 추정된 확률, 예측된 클래스 비해 신뢰성 저하 <br/><br/>\
# 활용 예시 <br/>\
- 스팸 필터링 : 텍스트 포함 기반 분류 / 이진 분류 <br/>\
- 비정상적인 상황 감지 : 침입, 이상행위 감지 / 이진 분류 <br/>\
- 의학적 질병 진단 : 종양 크기, 환자 나이 통한 암 여부 진단 / 이진 분류 <br/>\
- 문서 분류 : 문서 카테고리 분류 / 다중 분류 <br/><br/>\
# 보정 <br/>\
- 라플라스 스무딩 : 0 발생시 1로 최소값 보정 <br/>\
- 언더 플로우 현상 : 낮은 숫자 방지 위해 로그 취한 후 연산 <br/><br/>\
<a href = "https://needjarvis.tistory.com/621">참고 블로그</a>\
',
