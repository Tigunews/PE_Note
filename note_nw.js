...
//[패킷데이터 네트워크 계층 오류]- 해밍코드
'# 정의 : 패리티비트(Parity Bit)에 의한 오류검출 및 오류 정정까지 가능한 코드 <br/><br/>\
# 암기 : 2^p >= d + p + 1 <br/><br/>\
...
# 리우팅 헤더 취약성<br/>\
<img src = "./img/RoutingHeader.png" style = "max-width:100%; height:auto;"><br/><br/>\
1. 보안 취약성 : 우회 통한 제한된 접근 가능 <br/>\
2. 대응방안 <br/>\
- 규칙 설정 <br/>\
1) type 필드 값 0인 경우 패킷 차단 필터링 규칙 설정 <br/>\
2) 최종 목적지, 라우팅 헤더의 Address 필드 값 비교 규칙 설정 <br/>\
- 패킷의 라우팅 헤더에 포함된 최종 목적지 주소가 자신의 주소가 아닌 경우에는 패킷을 폐기 <br/><br/>\
# 멀티캐스트 취약성 <br/>\
<img src = "./img/MulticastAddress.png" style = "max-width:100%; height:auto;"><br/><br/>\
1. 보안 취약성 : 사이트 범위를 갖는 멀티캐스트 주소 <br/>\
2. 대응방안 <br/>\
- 경계 라우터에서 내부 IPv6 주소 필터링 <br/>\
- 주요 시스템에는 추즉이 어려운 IPv6 주소를 할당 사용 <br/>\
- 불필요한 ICMPv6 메시지 유입/유출 차단 <br/>\
- 침입차단 시스템은 최소한의 링크 로컬 멀티캐스트 트래픽만을 허용 <br/>\
- 사이트 범위의 목적지 주소 갖는 패킷 유입 차단 <br/><br/>\
# ICMPv6 통합 취약성<br/>\
1. 보안 취약성 : 처리 불가능한 특정 패킷들이 멀티캐스트 주소로 전송되면 에러 메시지를 송신자에게 보내는 것을 허용 <br/>\
2. 대응방안 : IPv4, IPv6 다른 보안정책 적용 <br/><br/>\
# 애니캐스트 취약점 <br/>\
<img src = "./img/AnycastAddressFiltering.png" style = "max-width:100%; height:auto;"><br/><br/>\
1. 보안 취약성 : 애니캐스트 그룹 멤버 통한 위장공격 및 서비스거부공격 <br/>\
2. 대응방안 : 애니캐스트 주소 필터링 <br/><br/>\
',
...
# 기술 : 서버, 호스팅 인프라, 응용 플랫폼, 가상화 관리자, 응용 플랫폼 서비스, 응용 <br/>\
-> 클라우드 및 가상화 기술, 대용량 표준 서버 기술, 응용 및 서비스 생태계, 모바일 데이터 전송 지연시간 1ms(0.001초) 로 최소화 <br/><br/>\
# 구성요소 <br/>\
<img src = "./img/MEC_0.png" style = "max-width:100%; height:auto;"><br/><br/>\
1. MEC 호스팅 인프라 <br/>\
- HW 자원 : 스위치, 스토리지 등 <br/>\
- 가상화 레이어 : HW 자원 가상화, 추상화 제공 <br/><br/>\
2. 응용플랫폼 <br/>\
- 가상화 관리자 : IaaS 컨트롤러-보안, 자원 샌드박스 제공, 멀티테넌시, 실시간 호스팅 <br/>\
- 응용플랫폼 서비스 : 미들웨어 서비스, MEC 서버 내 서비스 연결 Commnunication Service, 실시간 무선 및 NW 정보를 인가된 응용에 제공하는 RINS <br/><br/>\
# 동향 및 기대효과 <br/>\
...
'# 정의 : 6세대 네트워크 <br/><br/>\
# 특징 <br/>\
- 최대 전송속도 : 1000Gbps (5G 대비 50배)<br/>\
- 무선 지연시간 : 100 마이크로초 (5G 대비 1/10)<br/>\
- 상용화 시기 : 2030 예정 <br/>\
- 예상 서비스 : 초실감 확장 현실, 고정밀 모바일 홀로그램, 디지털 복제 서비스 <br/><br/>\
# 주요 트랜드 <br/>\
- 인공지능 활용 통신기술 확대 <br/>\
