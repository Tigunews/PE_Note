...
- 기각역 : 단층, 양측 <-> 채택역 <br/>\
- 유의 확률 <br/>\
- 1종 오류 : False Positive(긍정오류), FRR, True인데 False라고 판정 <br/>\
- 2종 오류 : False Negative(부정오류), FAR, False인데 True라고 전송 <br/>\
=> Biz 관점, 정상 고객을 거부하는 오류가 risk가 더 큼. 1종 오류 최소화 범위 내 2종 오류 최소화 방향으로 CER(Cross over Error Rate) 설정 필요<br/><br/>\
# 순서 : 귀무가설,대립가설 정의 > 검정통계 선택 > 기각역 선정 > p-value 산정 > 기각역 확인 > 가설 결정 <br/><br/>\
# 문제 풀이 <br/>\
(1) 가설의 수립 : 귀무가설(1분간 10건 거래 분석, FDS 도입효과 X -> H0 : u = 10), 대립가설(도입효과 O -> H1 : u > 10) <br/>\
(2) 유의 수준 설정 : 주어진 표본 평균이 표본 정규 분포 따른다고 가정, 유의 수준 5% = 1.645 <br/>\
(3) 유의확률 Z값 산출 : 주어진 표본 평균이 정규 분포를 따른다고 가정, Z = (10.4-10/(2.3/sqrt(50)) = 1.23 <br/>\
(4) 분석 : 1.645 > 1.23, 귀무가설 H0이 기각 X <br/>\
(5) 결과 해석 : 표본 평균 10.4 는 채택역으로 귀무가설 기각 X, FDS 도입은 통계적으로 실패를 의미 <br/><br/>\
# 통계학적 가설검정과 베이즈 이론의 관계 <br/>\
- 최근 조건부 확률 기반 베이즈 이론이 빅데이터 분석, 스팸필터 등에 많이 활용 <br/>\
- 통가검은 고정적 통계 표본기반, 베이즈는 새로운 정보 발견시 사후 확률 재계산하는 상대적 방식. 유연함 <br/><br/>\
<img src = "./img/가설검정.png" style = "max-width:100%; hegiht:auto;">\
',
