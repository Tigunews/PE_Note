...
'감리를 할때의 장점은 감리 수행을 통해 결함을 초반에 발견할 수 있는 장점이 있고, 단점은 프로젝트 초반에 집중해야할 요구사항 정의, 분석 설계등의 정보시스템 개발에 쏟아야 할 에포트를 감리준비에 집중해야 하는 단점이 있습니다. <br/>\
...
'Safety 악의성이 없는 위험으로부터의 안전을 말하고, Security는 악의적 위험으로부터의 안전을 말합니다. <br/>\
...
'4정규화는 다치 종속성이 제거된 정규화이고, 5정규화는 조인 종속성이 제거된 정규화 입니다. <br/>\
반정규화는 정규화로 인해 업무적 손실등 발생될 경우 이를 의도적으로 정규화를 낮추는 기법을 말합니다. 보통 5정규화는 반정규화 대상이 됩니다. \
',

'VPN은 암호화된 터널을 이용한 가상 사설명 네트워크 서비스 이고, SSL/TLS 프로토콜을 적용한 VPN을 SSL VPN이라 합니다. <br/>\
이외 에도 MPLS VPN과 IPSec VPN이 존재하며, 의료 보안관제인 의료 ISAC에서 의료기관과 센터간의 안전한 통신을 위해 SSL VPN 장비를 설치한 경험이 있습니다.\
',

'시스템, 서비스, 플랫폼은 확장 관계 입니다. 각각의 개념 설명을 하겠습니다. <br/>\
시스템은 체계적인 방법으로 이루어진 프로그램 입니다. <br/>\
서비스는 이러한 프로그램들의 집합체라 할 수 있고, <br/>\
플랫폼은 응용 프로그램이 실행될 수 있는 기초를 이루는 서비스의 집합이라 할 수 있습니다.\
',

'디지털 세는 그동안의 글로벌 빅테크 기업의 부당한 납세에 대한 글로벌한 대응이라고 생각합니다. <br/>\
이에 주요 납부대상인 미국에선 보복성 추가 세금 부과로 인한 무역 논쟁에 대한 국가적 대비가 필요하다고 생각합니다.\
',

'취약점과 약점에 대해 설명드리겠습니다. <br/>\
첫째 개념입니다. 취약점은 외부 공격으로부터의 보안 허점을 말하고, 약점은 SW 결함,실수,버그 등을 통한 오류를 말합니다. <br/>\
둘째 영향입니다. 취약점은 정보 악용의 영향으로 이어지고, 약점은 보안 사고로 영향을 끼칩니다. <br/>\
셋째 단계입니다. SDLC 측면에서 취약점은 운영단계의 리스크이고, 약점은 개발단계의 리스크입니다.\
',

'분류와 회귀 학습에 대해 설명드리겠습니다. <br/>\
첫째, 학습 방법측면입니다. 분류는 학습 데이터를 어떠한 그룹에 속하는지 찾아내는 학습방법입니다. 회귀는 결정자와 종속자의 관계를 찾아내는 학습방법입니다. <br/>\
둘째, 결과 측면입니다. 분류 학습의 결과는 레이블 그룹 안에 속하며, 회귀 학습의 결과는 방정식 형태로 결과가 도출됩니다.\
',

'FP의 데이터 유형은 데이터의 논리적 위치가 내부에 있느냐, 외부에 있느냐에 따라 ILF, EIF 로 나뉩니다. <br/>\
다음으로 트랜잭션 유형은 쿼리의 동작방식에 따라 EI, EO, EQ 로 나뉩니다.\
',

'SDLC 단계별 컴포넌트 산출물이 나오는 개발 방법론은 CBD 개발방법론입니다. 이에 착안하여 답변 드리겠습니다.<br/>\
첫째, 요구사항 수집 단계에선 Usecase Model 이 산출물로 존재합니다. <br/>\
둘째, 분석 및 설계 단계에서는 아키텍처 기술서, 인터페이스 명세서, DB 설계서 등이 산출물로 존재합니다. <br/>\
셋째, 구현 단계에서는 코딩규칙, 프로그래밍 가이드가 산출물로 존재합니다. <br/>\
넷쩨, 테스트 단계에서는 테스트 계획서가 산출물로 존재합니다.\
',

'플랫폼 규제에 대한 개인적 견해는 양날의 검이라고 생각합니다. <br/>\
해당 규제는 잘 사용했을시 공정한 시장경쟁을 유도하겠지만, 너무 이른 시기에 도입할 시 산업 성장을 저해 한다고 생각합니다. <br/>\
때문에, Positive 규제 방식의 접근이 필요한다고 생각합니다. \
',

'ASIL의 구성요소는 심각도인 Serverity, 노출 가능성인 Exposure, 통제 가능성인 Controllability 가 있습니다. 이를 통해 ABCD 등급으로 정의 됩니다.\
',

'메타버스는 Meta 와 Universe 의 합성어로 정치,경제,사회,문화 전반적 공존 가능한 가상세계를 말합니다. <br/>\
메타버스의 이슈는 사회적 이슈인 NFT 결합과 기술적 이슈인 메타버스 기기 확대 이슈가 존재합니다. <br/>\
추가적으로 제가 속한 의료 분야의 메타버스에 대해 설명드려도 되겠습니까? <br/><br/>\
의료분야에서의 메타버스 이슈는 의대생 교육 시뮬레이터, 비대면 원격진료, 디지털 치료제가 존재합니다.\
',

'FP는 ISO 14143에 의거한 기능 중심의 SW 규모 측정 방법입니다. 구성으론 데이터 기능과 트랜잭션 기능 지표가 있으며, DET,RET 는 데이터 기능 복잡도를 나타내는 지표입니다. <br/>\
DET 는 Attribute 의 차원수를 표현하고, RET 는 의미상 차원수를 표현합니다.\
',

'분석,설계시 간이법 통한 기능점수 산정 이후 명확해진 사실을 기반으로 하여 정통법을 통해 정당한 대가 산정과 평가를 위해 재계산 절차가 필요합니다.\
',

'첫째 개념 차이점입니다. 클라우드 컴퓨팅은 별도 장치 통한 리소스 활용 컴퓨팅이고, 엣지 컴퓨팅은 엣지단 리소스 활용 컴퓨팅입니다. <br/>\
둘째 데이터 센터 위치입니다. 클라우드 컴퓨팅은 중앙 데이터 센터와 소통하고, 엣지 컴퓨팅은 엣지 데이터 센터와 소통합니다. <br/>\
셋째 성능,보인 측면입니다. 클라우드 컴퓨팅은 성능 부하 증가, 공격 포인트 증가 특징이 있고, 엣지 컴퓨팅은 상대적 부하 감소, 프라이버시 정책 강화 측면의 특징이 존재합니다.\
',

'관리적, 물리적, 기술적 보안으로 나눌 수 있습니다. <br/>\
관리적 보안으론 CCTV 열람대장 운영, CCTV 운영 사실 공지 등이 있습니다. <br/>\
물리적 보안으론 물리적 망분리 운영, CCTV 회선 보안이 있습니다. <br/>\
기술적 보안으론 네트워크 보안, 운영 소프트웨어 보안이 있습니다.\
',

'작년 발의된 기술사법 개정안은 기술사의 권한과 책임에 대한 내용입니다. <br/>\
첫째, 권한에 대한 내용은 설계에 대한 대통령령 공공사업 발주시 기술사 서명 내용이며 <br/>\
둘째, 책임에 대한 내용은 해당 분야에서의 사고 발생시 책임 부과에 대한 내용입니다. <br/>\
이에 대한 정보관리기술사 측면에서의 생각은 추후 고착화될 분할발주와 4차 산업의 접목을 생각해보면, 각 단계별 안전에 대한 검증이 필요하므로 이는 꼭 개정되어야 할 법이라고 생각합니다.\
',

'SW 아키텍처랑 프로그램이나 시스템의 컴포넌트의 상호관계 구조와 이를 설계하기 위한 지침과 원리를 말합니다. <br/>\
분석 과정에서는 요구사항분석서,정의서가 주요산출물이고, 설계 과정에서는 유즈케이스 다이어그램, 인터페이스 정의서 등이 주요 산출물입니다.\
',

'망분리에 관한 관련규정은 공공, 민간, 금융부분에 대해 존재합니다. <br/>\
공공 분야는 국가정보보안 기본법, 민간은 개인정보보호법, 금융은 전자금융감독규정이 법적 근거입니다. <br/>\
실무에서 구축한 망분리는 지역 의료기관 관리자로 근무하였을 당시 내부망 네트워크와 외부망 네트워크를 물리적 망분리 구축 경험이 있습니다. 이에 관해 추가 설명 드려도 되겠습니까? <br/><br/>\
UTM 을 내부망과 외부망 전용으로 각각 두고, L2 Switch 를 물리적 장비 추가로 이를 해결하였고, 이를 통해 해결이 되지 않았을땐, Vlan을 이용하여 분리했습니다.\
',

'4차 산업혁명기술은 크게 Data, Network, AI 로 분류되는데, 이중 AI가 가장 파급력이 클 것이라고 생각합니다. <br/>\
그 이유를 기술적 측면과 법제도 측면으로 말씀드리겠습니다. <br/>\
먼저 기술적 측면입니다. AI 기술은 기존 단순 연산 패러다임에서 지능 학습 기반 기술 제공으로의 변화로 인한 파급력이 가장 클 것으로 예상됩니다. <br/>\
다음으로 정책적 측면입니다. 현재 국가 AI 발전 계획에서 전 산업 분야에 AI를 적용하고 있는 현황으로 미루어 보았을때, AI의 파급력이 가장 크다고 생각됩니다.\
',

'데이터 라벨링은 학습 데이터의 결과를 입력하는 과정을 말합니다. 데이터 라벨링에 있어서 중요한점은 학습 측면과 비즈니스 측면으로 나눌 수 있습니다. <br/>\
학습 측면으로는 AI의 효율적 학습을 가능케 함이 중요하고, 비즈니스 측면은 이러한 학습 결과가 비즈니스 니즈 반영에 최적화 될 수 있도록 라벨링을 설계하는 것이 중요하다고 생각듭니다.\
',

'DB 무결성은 데이터의 ACID 성질 확보 위해 무효갱신으로부터 데이터를 보호하는 성질입니다. <br/>\
그 종류론 개체, 참조, 속성, 키, 사용자 무결성이 존재합니다.\
',

'디지털 트윈은 가상 세계에 현실 세계의 요소를 반영한 동적 시뮬레이팅 기술입니다. <br/>\
세부 기술은 CPND 측면으로 설명드리겠습니다. <br/>\
첫째, 컨텐츠 측면으론 3D Engine, Simulation Modeling 기술이 존재합니다. <br/>\
둘째, 플랫폼 측면에선 Asset Store, Market Store 기술이 존재합니다. <br/>\
셋째, 네트워크 측면에선 IoT, CPS, 공간정보 기술등이 존재합니다. <br/>\
넷째, 디바이스 측면에선 AR Glass, HMD, 중력 장갑등이 존재합니다.\
',

'진료정보 공유시스템에서 개발 분야 PM으로 2년 2개월 근무한 경력이 있습니다. <br/>\
주요 일정으론 지역 진료정보 공유시스템 문서저장소 개발구축, 지역 병원 개발지원, 통합테스트 수행등이 있습니다.\
',

'먼저 RDBMS 적용 케이스입니다. 데이터 형태가 정형데이터로 구성되어 있고, 업무의 변동성이 적으며, OLTP 기반의 업무일 경우 RDBMS 로 구축하는 것이 좋습니다. <br/>\
당므으로 NoSQL 적용 케이스입니다. 데이터 형태가 정형,비정형 데이터로 구성되어 있고, 업무 변동성이 많으며, OLAP 기반의 업무일 경우 NoSQL 로 구축하는 것이 좋습니다. 이에 관련하여 실무 경험을 말씀드려도 되겠습니까? <br/><br/>\
진료정보 공유 시스템 구축당시 진료기반의 정형데이터와 영상기반의 비정형 데이터가 존재하였고, 정형데이터는 지역 문서저장소에 RDBMS 형태로, 비정형데이터는 G-Cloud 저장소에 NoSQL 기반으로 저장한 경험이 있습니다.\
',
];